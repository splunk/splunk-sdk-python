...
- Remove deprecated `wrap_socket` in `Context` class.
...
- Changed `splunklib/binding.py` Context class' constructor initialization to support default settings for encrypted http communication when creating the HttpLib object that it depends on. This is extracted from the keyword dictionary that is provided for its initialization. Encryption defaults to enabled if not specified.
...
- Made ResponseReader more stream-like, so that it can be wrapped in an
...
- The `Indexes.get_default` method has been added for returning the name of the
  default index.
...
record instead of the raw HTTP response.
