...
class IndexExporter(object):
    def __init__(self, index_name, earliest_time, latest_time, search_func, timeline_func,
        ...
        if os.path.exists(restart_path) and restart:
            restart_file = RestartFile(restart_path, rewind=rewind)
            plan = restart_file.plan
        else:
            plan = FreshBucket(
            ...
            restart_file = RestartFile(restart_path, plan=plan)

        self.n_rewound = restart_file.n_rewound
