...
class TestHandlingToolNameCollision(AITestCase):
    ...
    @patch(
        "splunklib.ai.agent._testing_local_tools_path",
        os.path.join(os.path.dirname(__file__), "testdata", "tool_with_long_description.py"),
    )
    @patch("splunklib.ai.agent._testing_app_id", "app_id")
    @ai_snapshot_test()
    async def test_token_limit_tools(self) -> None:
        ...
        async with Agent(
            ...
            with pytest.raises(TokenLimitExceededException, match="Token limit of 250 exceeded"):
                _ = await agent.invoke(msgs)
