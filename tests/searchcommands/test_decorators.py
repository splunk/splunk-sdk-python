...
class TestDecorators(TestCase):
    ...
    def test_configuration(self):
        ...
        for name, values, error_values in (
            ...
            ("distributed", (True, False), (None, "anything other than a bool")),
            (
                "generates_timeorder",
                (True, False),
                (None, "anything other than a bool"),
            ),
            ("generating", (True, False), (None, "anything other than a bool")),
            (
                "maxinputs",
                (0, 50000, sys.maxsize),
            ...
            ("requires_preop", (True, False), (None, "anything other than a bool")),
            ("retainsevents", (True, False), (None, "anything other than a bool")),
            ("run_in_preview", (True, False), (None, "anything other than a bool")),
            ("streaming", (True, False), (None, "anything other than a bool")),
            (
                "streaming_preop",
            ...
            for value in error_values:
                try:
                    ...
                    self.assertIsInstance(
                        error,
                        ValueError,
                        "Expected ValueError, not {}({}) for {}={}".format(
                            type(error).__name__, error, name, repr(value)
                        ),
                    )
                else:
                    self.fail(
                        "Expected ValueError, not success for {}={}".format(
                            name, repr(value)
                        )
                    )
    ...
    def test_option(self):
        ...
        self.assertListEqual(
            missing, [option.name for option in itervalues() if option.is_required]
        )
        self.assertListEqual(
            presets,
            [
                six.text_type(option)
        ...
        self.assertListEqual(
            ...
            [
                six.text_type(option)
                for option in itervalues()
                if six.text_type(option) != option.name + "=None"
            ],
        )
        ...
        for option in itervalues():
            ...
            self.assertEqual(
                validator.format(option.value),
                validator.format(validator.__call__(legal_value)),
            ...
            try:
                ...
                self.assertFalse(
                    "Expected ValueError for {}={}, not this {}: {}".format(
                        option.name, illegal_value, type(error).__name__, error
                    )
                )
            else:
                self.assertFalse(
                    "Expected ValueError for {}={}, not a pass.".format(
                        option.name, illegal_value
                    )
                )
        ...
        expected = (
            ...
            + ' integer="100" map="foo" match="123-45-6789" '
            ...
            "required_file="
            + json_encode_string(__file__)
            + ' required_integer="100" required_map="foo" '
