...
class TestDecorators(TestCase):
    ...
    def test_option(self):
        ...
        self.assertListEqual(
            missing, [option.name for option in options.values() if option.is_required]
        )
        self.assertListEqual(
            presets,
            [str(option) for option in options.values() if option.value is not None],
        )
        self.assertListEqual(
            presets,
            [
        ...
        for option in options.values():
            ...
            try:
                ...
                self.assertFalse(
                    f"Expected ValueError for {option.name}={illegal_value}, not a pass."
                )
        ...
        expected = (
            ...
            + ' float="99.9" integer="100" map="foo" match="123-45-6789" '
            ...
            "required_file="
            + json_encode_string(__file__)
            + ' required_float="99.9" required_integer="100" required_map="foo" '
