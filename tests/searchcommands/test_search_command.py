...
from splunklib.six import BytesIO, StringIO
...
def build_command_input(getinfo_metadata, execute_metadata, execute_body):
    input = "chunked 1.0,{},0\n{}".format(
        ...
        len(six.ensure_binary(execute_body)),
        execute_metadata,
        execute_body,
    )
...
class TestSearchCommand(TestCase):
    ...
    def test_process_scpv1(self):
        ...
        try:
            ...
            self.fail(
                "{0}: {1}: {2}\n".format(
                    type(error).__name__, error, result.getvalue().decode("UTF-8")
                )
            )
        ...
        try:
            ...
            self.fail(
                "Expected SystemExit, but caught {}: {}".format(
        ...
        try:
            ...
            self.fail(
                "Expected no exception, but caught {}: {}".format(
                    type(error).__name__, error
                )
            )
        else:
        ...
        self.assertEqual(
            command.service.authority, command.search_results_info.splunkd_uri
        )

        self.assertEqual(
            command.service.scheme, command.search_results_info.splunkd_protocol
        )
        ...
        self.assertEqual(
            command.service.namespace.app, command.search_results_info.ppc_app
        )
    ...
    def test_process_scpv2(self):
        ...
        dispatch_dir = os.path.join(
            basedir, "recordings", "scpv2", "Splunk-6.3", "countmatches.dispatch_dir"
        )
        ...
        command.search_results_info.search_metrics = (
            command.search_results_info.search_metrics.__dict__
        )
        command.search_results_info.optional_fields_json = (
            command.search_results_info.optional_fields_json.__dict__
        )
        ...
        input_file = build_command_input(
            getinfo_metadata, execute_metadata, execute_body
        )
        try:
            command.process(
                argv, input_file, ofile=result, allow_empty_input=False
            ...
            self.assertTrue(
                ...
                .decode("UTF-8")
                .__contains__(
                    "No records found to process. Set "
        ...
        try:
            command.process(
                argv, input_file, ofile=result
            )  # By default allow_empty_input=True
        except SystemExit as error:
            self.fail(
                "Unexpected exception: {}: {}".format(type(error).__name__, error)
            )
