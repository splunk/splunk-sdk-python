    ...
    def test_metadata(self):
        ...
        for c in collections:
            ...
            self.assertTrue(
                ...
                % (coll, found_access_keys, expected_access_keys),
            )
            self.assertTrue(
                found_fields_keys >= expected_fields_keys,
                msg="metadata.fields is missing keys on "
                + "%s (found: %s, expected: %s)"
                % (coll, found_fields_keys, expected_fields_keys),
            )
    ...
    def test_list(self):
        for coll_name in collections:
            ...
            self.assertEqual(
                expected,
                found,
                msg="on %s (expected: %s, found: %s)" % (coll_name, expected, found),
            )
    ...
    def test_paging(self):
        for coll_name in collections:
            ...
            while offset < total:
                ...
                offset += count
                self.assertTrue(
                    count == page_size or offset == total, msg="on %s" % coll_name
                )
